...
### 1-1. 🚀 단기 급등 예외 처리 (Strong Momentum Exception) - v3.3 최종
장기 하락장이라도 **강력한 단기 상승 모멘텀**이 감지되면 진입을 허용합니다.

| 조건                    | 임계값           | 효과                                          |
| ----------------------- | ---------------- | --------------------------------------------- |
| **5분봉 급등**          | +1.5% 이상       | 중기 모멘텀 확인                              |
| **1분봉 일관성**        | 5개 중 3개 상승  | 5분 사이 **지속적 상승** 확인 (오락가락 방지) |
| **4시간봉 플러스 전환** | 0% 초과 (플러스) | 중기 추세 전환 확인                           |
| **매수 우위**           | 55% 이상         | 매수 세력 우위 확인                           |
| **낮은 피로도**         | 40 이하          | 상승 둔화 없음 확인                           |
| **모든 조건 만족 시**   | -                | **✅ 장기하락 차단 무시** 🚀                    |
| **판단 주기**           | 10초             | **빠른 반응 속도** ⚡                          |
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
기존 v3.2 (차단됨):
  5m: +1.87% ✅  |  4h: +1.07% ✅  |  3일: -2.05% ❌
  → 🚫 장기하락 차단 (진입 불가)

v3.3 케이스 1 (진입 허용 - 강한 모멘텀):
  5m: +1.87% ✅  |  1m일관: 3/3 ✅  |  4h: +1.07% ✅
  매수: 68% ✅  |  피로: 25 ✅  |  3일: -2.05% (무시)
  1분봉: [+0.4%, +0.3%, +0.5%, +0.4%, +0.3%] 모두 상승 ✅
  → 🚀 단기 급등 감지 - 예외 적용 (진입!)
  
v3.3 케이스 2 (차단 - 오락가락):
  5m: +1.92% ✅  |  1m일관: 2/3 ❌  |  4h: +1.15% ✅
  매수: 62% ✅  |  피로: 28 ✅  |  3일: -2.10%
  1분봉: [+0.8%, -0.2%, +1.0%, -0.1%, +0.5%] 불안정 ❌
  → ❌ 1분봉 일관성 불만족 - 진입 불가
  
v3.3 케이스 3 (차단 - 매도 우위):
  5m: +1.63% ✅  |  1m일관: 3/3 ✅  |  4h: +0.95% ✅
  매수: 48% ❌  |  피로: 28 ✅  |  3일: -2.20%
  → ❌ 매수 우위 조건 불만족 (55% 미만)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
💡 **핵심**: 
- **5분봉**: 중기 모멘텀 확인 (기존 유지)
- **1분봉 일관성**: 5분 사이 **지속적으로 상승**하는지 체크
  - 5개 1분봉 중 **3개 이상** 상승 → 강한 모멘텀 ✅
  - 2개 이하 상승 → 오락가락, 불안정 ❌
- **10초 판단**: 기존 30초 → 10초로 **3배 빠른** 기회 포착
- **매수 우위 확인**: 실제 매수 세력이 우세해야 진입 (매도 압력 회피)
- **피로도 체크**: 상승 둔화 및 모멘텀 소진 시 진입 보류
- **5중 안전장치**: 5가지 조건을 모두 만족해야 예외 적용
- **실시간 모니터링**: 이미 구독 중인 1분봉/5분봉 데이터를 활용하여 API 추가 부담 없음

### 2. ⚡ 스마트 데이터 로딩 (Smart Data Loading)
- **로컬 캐싱**: `data/` 디렉토리에 캔들 데이터를 **CSV**로 저장하여 영구 보관
- **고속 초기화**: 재시작 시 부족한 데이터(Gap)만 API로 요청 (초기 로딩 3초 이내)
- **API 절약**: 불필요한 전체 데이터 요청을 제거하여 API 리밋 방지
- **가독성**: CSV 형식으로 데이터 확인 및 분석 용이
...
# 패키지 설치
pip install websockets aiohttp python-dotenv PyJWT requests redis pandas
```
...
├── data/                # 캔들 데이터 캐시 (CSV)
│   ├── KRW-BTC_1m.csv   # BTC 1분봉 데이터
│   ├── KRW-BTC_5m.csv   # BTC 5분봉 데이터
│   ├── KRW-BTC_15m.csv  # BTC 15분봉 데이터
│   └── ...              # 기타 마켓 데이터
├── api.md               # API 문서
└── README.md            # 이 파일
```
