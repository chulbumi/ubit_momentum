...
├── crypto_dashboard.py  # 🆕 btop 스타일 터미널 대시보드
...
## 🎛️ Crypto Dashboard (터미널 대시보드)

btop 스타일의 터미널 기반 암호화폐 대시보드입니다.

### 실행 방법
```bash
./run_dashboard.sh
# 또는
source venv/bin/activate && python crypto_dashboard.py
```

### 주요 기능
- **실시간 가격 모니터링**: WebSocket으로 실시간 시세 수신
- **텍스트 기반 미니 차트**: 가격 히스토리를 시각적으로 표현
- **보유 자산 현황**: 포트폴리오 평가금액 및 수익률
- **시장 지표**: 상승/하락 종목 수, TOP/BOTTOM 변동률
- **명령어 기반 인터페이스**: 마켓 추가/제거, 매수/매도 등

### 명령어 목록

| 명령어     | 설명                   | 예시                  |
| ---------- | ---------------------- | --------------------- |
| `/add`     | 마켓 추가              | `/add KRW-DOGE`       |
| `/remove`  | 마켓 제거              | `/remove KRW-XRP`     |
| `/top N`   | 거래대금 상위 N개 추가 | `/top 10`             |
| `/buy`     | 시장가 매수            | `/buy KRW-BTC 100000` |
| `/sell`    | 시장가 매도 (전량)     | `/sell KRW-ETH`       |
| `/balance` | 잔고 새로고침          | `/balance`            |
| `/clear`   | 로그 지우기            | `/clear`              |
| `/quit`    | 종료                   | `/quit`               |
| `/help`    | 도움말                 | `/help`               |
...
```
┌─────────────────────────────────────────────────────────────┐
│ 🎛️ Crypto Dashboard │ 2026-01-20 08:00:00 │ WebSocket: 🟢   │
├──────────────────┬──────────────────────────────────────────┤
│ 💰 보유 자산      │ 📈 시장 지표                              │
│ KRW: 1,000,000원 │ BTC: 150,000,000 (+2.50%)                │
│ BTC:   150,000원 │ 상승: 15 │ 하락: 8 │ 보합: 2             │
├──────────────────┴──────────────────────────────────────────┤
│ 📊 시세 모니터링                                             │
│   마켓   │    현재가    │  변동률  │  24h 거래대금  │ 미니차트│
│ KRW-BTC │ 150,000,000  │  +2.50%  │     1,234.5억  │ ▄▅▆▇███ │
│ KRW-ETH │   4,500,000  │  -1.20%  │       456.7억  │ █▇▆▄▃▂▁ │
├──────────────────────────────────────────────────────────────┤
│ 📋 로그                                                      │
│ 08:00:01 WebSocket 연결됨                                    │
│ 08:00:05 추가됨: KRW-DOGE                                    │
├──────────────────────────────────────────────────────────────┤
│ > /add KRW-SOL                                               │
└──────────────────────────────────────────────────────────────┘
```
